    ...
    <div>
          ...
          {segments.sort((a, b) => b.count - a.count).map((s: any) => (
            <button key={s.segment} onClick={() => setSegmentFilter(segmentFilter === s.segment ? "" : s.segment)}
              className={`shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition ${
                segmentFilter === s.segment ? "border-red-300 bg-red-50 text-red-600" : "border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:bg-gray-50"
              }`}>
      ...
      <Table headers={["", "Contato", "Segmento", "Pedidos", "Gasto Total", "Última Compra", "Cidade"]}
        ...
        {loading ? (
          ...
          <tr key={c.id} className="hover:bg-gray-50 cursor-pointer transition" onClick={() => openDetail(c.id)}>
            ...
            <td className="px-4 py-3 text-sm text-gray-700">{c.totalOrders || 0}</td>
            <td className="px-4 py-3 text-sm font-medium text-gray-900">{formatCurrency(c.totalSpent)}</td>
            <td className="px-4 py-3 text-xs text-gray-500">{formatRelativeTime(c.lastOrderAt)}</td>
            <td className="px-4 py-3 text-xs text-gray-500">{c.city ? `${c.city}/${c.state}` : "—"}</td>
          </tr>
        ))}
      </Table>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
                <p className="text-lg font-bold text-gray-900">{selectedContact.totalOrders || 0}</p>
                <p className="text-xs text-gray-500">Pedidos</p>
              </div>
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
                <p className="text-lg font-bold text-gray-900">{formatCurrency(selectedContact.totalSpent)}</p>
                <p className="text-xs text-gray-500">Total Gasto</p>
              </div>
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
                <p className="text-lg font-bold text-gray-900">{formatCurrency(selectedContact.avgOrderValue)}</p>
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
              <h4 className="text-xs font-semibold text-gray-500 mb-3">Score RFM</h4>
                ...
                {[
                  ...
                  <div key={m.label} className="flex-1">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-xs font-bold text-gray-700">{m.value || 0}/5</span>
                    </div>
                    <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                      <div className="h-full bg-red-500 rounded-full" style={{ width: `${((m.value || 0) / 5) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            {selectedContact.orders && selectedContact.orders.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-500 mb-2">Últimos Pedidos</h4>
                <div className="space-y-2">
                  {selectedContact.orders.slice(0, 5).map((o: any) => (
                    <div key={o.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 border border-gray-100 rounded-lg">
                      <div>
                        <span className="text-sm text-gray-800">#{o.orderNumber}</span>
                        <span className="text-xs text-gray-400 ml-2">{formatDate(o.placedAt)}</span>
                      </div>
            ...
            {selectedContact.interactions && selectedContact.interactions.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-500 mb-2">Histórico</h4>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {selectedContact.interactions.slice(0, 10).map((i: any) => (
                    <div key={i.id} className="flex items-start gap-3 py-2">
                      ...
                      <div>
                        <p className="text-xs text-gray-600">{i.content || i.type}</p>
                        <p className="text-[10px] text-gray-400">{formatRelativeTime(i.createdAt)} · {i.channel}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <Modal open={tagModal} onClose={() => setTagModal(false)} title="Gerenciar Tags" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-gray-500">{selectedIds.length} contatos selecionados</p>
          <div>
            <label className="text-xs font-medium text-gray-600 mb-1.5 block">Nome da Tag</label>
            <input value={tagValue} onChange={(e) => setTagValue(e.target.value)}
              placeholder="ex: vip, black-friday, inativo"
              className="w-full px-3 py-2 rounded-lg bg-white border border-gray-300 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-red-500" />
          </div>
