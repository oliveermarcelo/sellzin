    ...
    <div>
            ...
            {conversion.map((c: any, i: number) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[10px] text-gray-500">{rate.toFixed(0)}%</span>
                  <div className="w-full flex gap-1" style={{ height: "80px" }}>
                    <div className="flex-1 bg-gray-100 rounded-sm" style={{ height: `${Math.min((c.total / Math.max(...conversion.map((x: any) => x.total), 1)) * 100, 100)}%`, marginTop: "auto" }} />
                  ...
                  <span className="text-[10px] text-gray-400">{new Date(c.month).toLocaleDateString("pt-BR", { month: "short" })}</span>
                </div>
              );
            })}
          </div>
          <div className="flex items-center gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-[10px] text-gray-500"><span className="w-2 h-2 rounded-sm bg-gray-100" /> Abandonados</span>
            <span className="flex items-center gap-1.5 text-[10px] text-gray-500"><span className="w-2 h-2 rounded-sm bg-emerald-500/50" /> Recuperados</span>
          </div>
        </div>
      )}
          ...
          <tr key={c.id} className="hover:bg-gray-50 transition">
            <td className="px-4 py-3 w-10">
              <input type="checkbox" checked={selectedCarts.includes(c.id)}
                onChange={() => toggleSelect(c.id)} className="rounded bg-gray-100 border-gray-300" />
            </td>
            <td className="px-4 py-3">
              {c.contact ? (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm text-gray-700">{c.contact.firstName} {c.contact.lastName}</p>
                    <p className="text-xs text-gray-400">{c.contact.phone || c.contact.email}</p>
                  </div>
                </div>
              ) : (
                <div><p className="text-sm text-gray-600">{c.email || c.phone || "Anônimo"}</p></div>
              )}
            </td>
            <td className="px-4 py-3">
              <div className="max-w-xs">
                {(c.items || []).slice(0, 2).map((item: any, i: number) => (
                  <p key={i} className="text-xs text-gray-500 truncate">{item.name} ({item.quantity}x)</p>
                ))}
                {(c.items || []).length > 2 && (
                  <p className="text-[10px] text-gray-400">+{c.items.length - 2} itens</p>
                )}
              </div>
            </td>
            <td className="px-4 py-3 text-sm font-semibold text-gray-700">{formatCurrency(c.total)}</td>
            <td className="px-4 py-3">
              <span className={`text-xs font-medium ${c.recoveryAttempts > 0 ? "text-amber-400" : "text-gray-400"}`}>
                {c.recoveryAttempts || 0}/3
              </span>
            </td>
            <td className="px-4 py-3 text-xs text-gray-500">{formatRelativeTime(c.abandonedAt)}</td>
          </tr>
        ))}
      </Table>
      ...
      <Modal open={recoveryModal} onClose={() => setRecoveryModal(false)} title="Recuperar Carrinhos" size="md">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 mb-1.5 block">Cupom de desconto (opcional)</label>
            <div className="flex items-center gap-2">
              <Gift className="w-4 h-4 text-gray-400" />
              <input value={couponCode} onChange={(e) => setCouponCode(e.target.value)}
                placeholder="ex: VOLTA10"
                className="flex-1 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm text-white placeholder:text-gray-400 focus:outline-none focus:border-indigo-500/50" />
            </div>
          </div>

          <div>
            <label className="text-xs font-medium text-gray-600 mb-1.5 block">Mensagem personalizada (opcional)</label>
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <h4 className="text-xs font-semibold text-gray-600 mb-2">Fluxo de recuperação</h4>
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-gray-400 ml-2">{step.msg}</span>
                  </div>
                </div>
              ))}
            </div>
